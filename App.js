...
import {
  Bell, Calendar, User, Search, Filter, Plus, Edit, Trash2, LogOut,
  BookOpen, Clock, Building, Users, Eye, X
} from 'lucide-react';
import './App.css';
...
//     return { data: { token: data.token, admin: data.admin } };
//   },
...
//     const response = await fetch(`http://localhost:9000/api/notices?${query.toString()}`, {
//       headers: {
...
const mockAPI = {
  ...
  createNotice: async (notice) => {
    const token = localStorage.getItem('token');
    const response = await fetch('http://localhost:9000/api/notices', {
      method: 'POST',
      headers: {
  ...
  deleteNotice: async (id) => {
    const token = localStorage.getItem('token');
    const response = await fetch(`http://localhost:9000/api/notices/${id}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`
      }
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 mb-8">
              ...
              <div className="flex justify-center space-x-4">
                ...
                <button
                  ...
                  className="bg-gradient-to-r from-purple-600 to-blue-500 text-white px-8 py-4 rounded-lg hover:from-purple-700 hover:to-blue-600 transition-all duration-200 flex items-center space-x-2"
                >
...
const StudentPanel = () => {
  ...
  return (
    <div>
      <Header isAdmin={false} />
      <StudentNoticeBoard notices={notices} onFilterChange={setFilters} filters={filters} />
    </div>
  );
};
...
const AdminPanel = () => {
  ...
  const fetchNotices = async () => {
    try {
      const response = await mockAPI.getNotices();
      setNotices(response.data);
    } catch (error) {
      console.error('Error fetching notices:', error);
    } finally {
